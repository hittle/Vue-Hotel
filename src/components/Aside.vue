  ...
  <el-aside :style="`width:${asideStyle}px;height:100%;`">
    ...
    >
     ...
     <router-link to='/test' >
      ...
      <router-link to='/history' v-if="user.identity ==='admin' || user.identity === 'staff'">
        <el-menu-item index="3">
          <i class="el-icon-menu"></i>
          <span slot="title">历史纪录查询</span>
        </el-menu-item>
      </router-link>
      <router-link to='/statistics' v-if="user.identity ==='admin'">
        <el-menu-item index="4">
          <i class="el-icon-setting"></i>
          <span slot="title">收入统计汇总</span>
        </el-menu-item>
      </router-link>
      <router-link to='/roomInfor' v-if="user.identity ==='admin'">
        <el-menu-item index="5">
          <i class="el-icon-setting"></i>
          <span slot="title">房间信息管理</span>
        </el-menu-item>
      </router-link>
      <router-link :to="{name:'custInfo',params:{identity:'customer'}}" v-if="user.identity ==='admin'"> 
        <el-menu-item index="6">
          <i class="el-icon-setting"></i>
          <span slot="title">用户信息管理</span>
        </el-menu-item>
      </router-link>
      <router-link :to="{name:'staffInfo',params:{identity:'staff'}}"  v-if="user.identity ==='admin'">
