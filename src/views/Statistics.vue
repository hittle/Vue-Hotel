    ...
    getStatistics(){
      this.getRequest('/getStatistic').then(res=>{
        ...
        for(var i in arr) {    //对同一天的数据进行处理
          var key=arr[i].liveDateView.substring(0,10);
          if(tmpObj[key]){   //如果key值存在，则对同一天的盈利房费进行相加
            tmpObj[key].value+=Number(arr[i].totalMoney);
          }else{   //如果不存在，则新建一个对象
            tmpObj[key]={};
            tmpObj[key].value=Number(arr[i].totalMoney);
            tmpObj[key].name=arr[i].liveDateView.substring(0,10);
          }

          var typeKey=arr[i].roomType;
          if(tmpRoomType[typeKey]){
            tmpRoomType[typeKey].value+=1;
          }else{
            tmpRoomType[typeKey]={};
            tmpRoomType[typeKey].value=1;
            tmpRoomType[typeKey].name=arr[i].roomType;
